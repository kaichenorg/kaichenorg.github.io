<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "GET") {
		$keyfile = 'randomfiles/' . test_input($_GET["key"]);
		if (!is_file($keyfile))
		{
			header("Location: err.html"); 
			exit;
		}
		$createtime=filectime($keyfile);
		if (time() - $createtime > 2*24*60*60)
		{
			header("Location: err.html"); 
			exit;
		}
		$myfile = fopen($keyfile, "r") or die("Unable to open file!");
		$downloadfile = fread($myfile, filesize($keyfile));
		fclose($myfile);
		//downfile($downloadfile);
		$obj = new FileDownload();  
		$obj->download($downloadfile,'',true); 
	}
	else
	{
		header("Location: err.html"); 
